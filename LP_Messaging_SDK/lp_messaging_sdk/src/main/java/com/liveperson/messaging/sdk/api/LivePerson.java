...
public class LivePerson {
    ...
    public static void initialize(Context context, final InitLivePersonProperties initProperties) {
		...
		// Setting an UncaughtExceptionHandler, sending the hostapp's handler to be able to invoke it later.
		...
		InitLivePersonCallBack callbackForMonitoringInit = new InitLivePersonCallBack() {
			...
			public void onInitSucceed() {
				...
				LPMobileLog.d(TAG, "onInitSucceed: lp_messaging_sdk module version = " + BuildConfig.VERSION_NAME);
        ...
        if (!isValidState()) {
            ...
            MessagingUIFactory.getInstance().init(applicationContext, new MessagingUiInitData(initProperties, getSDKVersion(), initProperties.getInterceptors()), new MessagingUiConfiguration(null));
    ...
    /**
     * Set debug mode for SDK logs.
     * If in released version, Liveperson logs will be disabled.
     * */
    private static void setLogDebugMode() {
    ...
    public static Fragment getConversationFragment(LPAuthenticationParams lpAuthenticationParams, ConversationViewParams params‎) {
        if (!isValidState()) {
            LPMobileLog.e(TAG, "getConversationFragment- not initialized!");
            return null;
        }
    ...
    public static void registerLPPusher(String brandId, String appId, String gcmToken) {
        ...
        MessagingFactory.getInstance().getController().registerPusher(brandId, appId, gcmToken, null,null);
    }
    /**
     * Register LivePerson pusher service
     *
    ...
    public static void registerLPPusher(String brandId, String appId, String gcmToken, LPAuthenticationParams authenticationParams,final ICallback<Void, Exception> registrationCompletedCallback) {
        ...
        MessagingFactory.getInstance().getController().registerPusher(brandId, appId, gcmToken,authenticationParams, registrationCompletedCallback);
    }
